...
# =====================================================
# FONCTIONS DE CHIFFREMENT TTU-MC³
# =====================================================
...
def decrypt_text(ciphertext: str) -> str:
    ...
    try:
        ...
        return "🔐 Message illisible (clé incorrecte)"

# =====================================================
# FONCTIONS TTU-MC³ - ANALYSE DE STABILITÉ
# =====================================================
def compute_ttu_stability(phi_m, phi_c, phi_d):
    """Calcule la stabilité triadique selon TTU-MC³."""
    # Fonction de Lyapunov simplifiée
    V = phi_m**2 + phi_c**2 + phi_d**2
    # Taux de dissipation
    dissipation = -0.1 * (phi_m + phi_c + phi_d)
    # Cohérence triadique
    coherence = phi_m * phi_c * phi_d / max(V, 0.001)
    return {
        "lyapunov": V,
        "dissipation": dissipation,
        "coherence": coherence,
        "stability": 1.0 / (1.0 + abs(dissipation))
    }

def visualize_ttu_state(phi_m, phi_c, phi_d):
    """Génère une visualisation 3D de l'état triadique."""
    fig = go.Figure(data=[
        go.Scatter3d(
            x=[0, phi_m],
            y=[0, phi_c],
            z=[0, phi_d],
            mode='lines+markers',
            line=dict(color='#ff9d00', width=5),
            marker=dict(size=10, color=['red', '#ff9d00'])
        )
    ])
    fig.update_layout(
        scene=dict(
...
def feed_page():
    ...
    st.markdown("""
        ...
        .ttu-meter {
            background: linear-gradient(90deg, #ff9d00, #ff4b4b);
            height: 4px;
            border-radius: 2px;
            margin: 5px 0;
        }
    ...
    with col_m1:
        st.metric("🧠 Mémoire Φ_M", f"{np.random.uniform(0.7, 1.0):.3f}")
    with col_m2:
        st.metric("🔄 Cohérence Φ_C", f"{np.random.uniform(0.5, 0.9):.3f}")
    with col_m3:
        st.metric("💨 Dissipation Φ_D", f"{np.random.uniform(0.1, 0.4):.3f}")
    st.markdown('<div class="ttu-meter"></div>', unsafe_allow_html=True)

    # Tendances
    st.markdown('<p class="trending-title">🔥 Tendances TTU</p>', unsafe_allow_html=True)
    try:
        tips_24h = supabase.table("tips").select("post_id, amount").gte("created_at", (datetime.now() - timedelta(days=1)).isoformat()).execute()
        if tips_24h.data:
            tip_sums = {}
            for tip in tips_24h.data:
                tip_sums[tip['post_id']] = tip_sums.get(tip['post_id'], 0) + tip['amount']
            post_ids = list(tip_sums.keys())
            trending_posts = supabase.table("posts").select("id, user_id, text, media_path, profiles!inner(username, profile_pic)").in_("id", post_ids).execute()
    ...
    with st.spinner("🌊 Chargement..."):
        try:
            posts = supabase.table("posts").select("*, profiles!inner(username, profile_pic)").order("created_at", desc=True).limit(30).execute()
        except Exception as e:
    ...
    for post in posts.data:
        with st.container(border=True):
            ...
            # Suppression pour l'auteur ou admin
...
def profile_page():
    ...
    with tab_stats:
        ...
        with col1:
            try:
                ...
                posts_count = 0
            st.markdown(f'<div class="metric-card"><div class="metric-value">{posts_count}</div><div class="metric-label">Publications</div></div>', unsafe_allow_html=True)
        with col2:
            try:
                followers = supabase.table("follows").select("*", count="exact").eq("followed", user.id).execute().count
            except:
                followers = 0
            st.markdown(f'<div class="metric-card"><div class="metric-value">{followers}</div><div class="metric-label">Abonnés</div></div>', unsafe_allow_html=True)
        with col3:
            try:
                following = supabase.table("follows").select("*", count="exact").eq("follower", user.id).execute().count
            except:
                following = 0
            st.markdown(f'<div class="metric-card"><div class="metric-value">{following}</div><div class="metric-label">Abonnements</div></div>', unsafe_allow_html=True)
        with col4:
            try:
                user_posts = supabase.table("posts").select("id").eq("user_id", user.id).execute()
                if user_posts.data:
                    post_ids = [p['id'] for p in user_posts.data]
                    likes_received = supabase.table("likes").select("*", count="exact").in_("post_id", post_ids).execute().count
                else:
                    likes_received = 0
            except:
                likes_received = 0
            st.markdown(f'<div class="metric-card"><div class="metric-value">{likes_received}</div><div class="metric-label">Likes reçus</div></div>', unsafe_allow_html=True)
            ...
            if wallet.data:
                ...
                with col_w3:
                    if wallet.data.get('last_reward_at'):
                        ...
                        st.metric("Prochain minage", f"{hours}h {minutes}m")
        except Exception:
        ...
        try:
            ...
            if last_posts.data:
                ...
                for p in last_posts.data:
                    ...
                    text_preview = p['text'][:50] + "..." if p.get('text') and len(p['text']) > 50 else p.get('text', '')
                    st.caption(f"{media_icon} {text_preview} - {p['created_at'][:10]}")
        except:
            pass
        st.divider()
        try:
            ...
            if last_msgs.data:
                ...
                for m in last_msgs.data:
                    ...
                    text_preview = m['text'][:30] + "..." if m.get('text') and len(m['text']) > 30 else m.get('text', '')
                    st.caption(f"🗣️ Dans {tunnel_name}: {text_preview} - {m['created_at'][:16]}")
        except:
            pass
        st.divider()
        try:
    ...
    with tab_tunnels:
        ...
        try:
            tunnels = supabase.table("tunnel_members").select("tunnel_id, tunnels(name, k_hash, created_at, creator_id, description)").eq("user_id", user.id).execute()
            if tunnels.data:
    ...
    with tab_edit:
        ...
        with st.expander("📸 Changer ma photo", expanded=False):
            uploaded_file = st.file_uploader("Choisir une image (max 5 Mo)", type=["png", "jpg", "jpeg"], key="avatar_upload")
            if uploaded_file:
                if uploaded_file.size > 5 * 1024 * 1024:
                    st.error("Image trop volumineuse (max 5 Mo).")
                else:
                    if st.button("✅ Uploader cette photo"):
                        try:
        ...
        with st.form("edit_profile_form"):
            ...
            new_bio = st.text_area("Bio", value=profile.get("bio", ""), max_chars=160, help="160 caractères maximum")
            new_location = st.text_input("Localisation", value=profile.get("location", ""))
            submitted = st.form_submit_button("💾 Sauvegarder les modifications", use_container_width=True, type="primary")
            if submitted:
                try:
    ...
    with tab_vault:
        ...
        st.markdown("""
        <div style="background: linear-gradient(135deg, #ff9d0011, #ff4b4b11); padding: 15px; border-radius: 10px; margin-bottom: 20px;">
        ...
        try:
            ...
            if keys_history.data:
                ...
                for k in keys_history.data:
                    ...
                    tunnel_name = k.get('tunnel_name', 'Inconnu')
                    col_k3.caption(f"🚇 {tunnel_name}")
            else:
                st.info("Aucune clé enregistrée. Utilisez un tunnel pour générer une clé.")
        except Exception:
            st.info("Module de coffre en cours d'activation.")
...
def messages_page():
    ...
    with st.sidebar:
        ...
        st.divider()
        real_time = st.toggle("📡 Mode Temps Réel", value=True, help="Actualisation automatique (intervalle adaptatif)")
    ...
    selected_t_id = st.selectbox("Sélectionner le canal", options=list(t_options.keys()), format_func=lambda x: t_options[x], index=default_index, key="tunnel_selector")
...
def marketplace_page():
    ...
    with st.sidebar:
        ...
        search = st.text_input("Rechercher un article", value=st.session_state.search_query, key="search_input", placeholder="Nom, description...")
...
def wallet_page():
    ...
    st.divider()
    st.subheader("📜 Activité récente")
    st.info("L'historique détaillé des transactions sera bientôt disponible.")
